...
namespace ArduinoProject
{
    class TrayIcon
    {
        ...
        {
            using (Image image = new Bitmap(3, 3))
        ...
        private void TimerTick (object sender, EventArgs e)
        {
            ...
            String ArduinoProject = (powerStatus.BatteryLifePercent * 100).ToString();
            bool isCharging = SystemInformation.PowerStatus.PowerLineStatus == PowerLineStatus.Online;
            String bitmapText = isCharging ? ArduinoProject + "*" : ArduinoProject;
            using (Bitmap bitmap = new Bitmap(GetTextBitmap(bitmapText, new Font(font, fontSize), Color.White)))
            {
                ...
                try
                {
                    using (Icon icon = Icon.FromHandle(intPtr))
                    {
                        ...
                        String toolTipText = ArduinoProject + " % " + (isCharging ? "Charging" : "");
